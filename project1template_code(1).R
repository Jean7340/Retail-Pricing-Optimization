...
#setwd("L:/Takeaki/Google drive/Past classes/Rochester Simon/2025 Spring A pricing analytics/Project 1")
...
#each car model - year combination gets assigned a unique value of fixed effect.
...
#the intercepts and fixed effects.
#These intercept values are not absolutely necessary for Project 1. However,
#if you find yourself needing one - here's how to retrieve it.

#To get the values of the fixed effects, we use "getfe" function.
#For instance, consider recovering fixed effect values from "reg3_2" above.
fe=getfe(reg3_2)
...
#Find where it is located - I'll use "match" function (no need to memorize it - just copy and paste it).
idc <- match('90',fe$idx)
fe90=fe[idc,1]
#fe90 is the value of fixed effect for year 1990, i.e., the intercept values you'll use to predict
#the demand for 1990.
...
#fe90_3 is the intercept value for the demand in market 3 in 1990.
...
#"seq" here generates a set of equidistant points between 0 and 50000 with the distance of 100.
...
#See the use of "pricespace" - we are evaluating the predicted quantity at all prices in the space of "pricespace".
#Also, see "fe90_3" used as the intercept. That's the value of the intercept we retrieved above.
...
#The first two inputs are the X- and Y-axis of the plot. Other values are optional and are for better looking.
...
# Z is our instruments for log(P). Importantly, if you instrument for log(P),
# we need to drop log(P) from the first part of the code (so the first part has only X in it).
